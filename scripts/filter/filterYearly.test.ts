...
describe("Testing monthly filter", () => {
  test("should return monthly values", () => {
    expect(filterYearly(completedTodayTask)).toBe("completed");
    expect(filterYearly(overdueTodayTask)).toBe("overdue");
    expect(filterYearly(todayTask)).toBe("today");
    expect(filterYearly(upcomingTask)).toBe("upcoming");
    expect(filterYearly(overdueB4Task)).toBe("overdue");
    expect(filterYearly(completedTodayB4Task)).toBe("completed");
  });
});
